...
### Configured Payment Gateway & Settlement
- **Destination UPI ID**: `9182916879@ybl`
- **Payment Provider**: Official Razorpay integration with server-side HMAC-SHA256 signature verification & idempotent webhooks.
...
- **Multi-Step Checkout Modal**:
  ...
  - **Step 4: Payment Selection**: `Pay at Counter` vs `Pay Online via UPI (Razorpay)`.
  - **Step 5: Review & Confirm**: Final itemized order breakdown and server-verified total.

### 2. Live Order Tracking (`/order.html`)
- Customer order confirmation receipt with unique human-readable order number (e.g. `CAF-1001`).
- Dynamic status progression: `Received` &rarr; `Preparing` &rarr; `Ready` &rarr; `Completed`.
- Real-time client polling every 4 seconds.
...
- **Live Orders Board**: Real-time incoming orders queue, sound chime alert, status advance buttons (`Accept & Prepare`, `Mark Ready`, `Complete Order`), and `Mark as Paid` action for counter orders.
- **Instant Availability Toggle**: Toggle any product between `AVAILABLE` and `UNAVAILABLE` with a single click—immediately reflected on customer menu and backend validation.
...
- The backend recalculates order totals from database records and creates Razorpay orders in paise (`amount * 100`).
- Server-side HMAC-SHA256 signature verification prevents forged payment confirmations.
- Webhooks verify `x-razorpay-signature` and enforce idempotency using the `webhook_events` table.
